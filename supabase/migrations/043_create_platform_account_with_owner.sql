-- ============================================================
-- 043_create_platform_account_with_owner.sql
--
-- Platform-owner flow to provision a client organisation with an
-- owner account already created via auth.admin.createUser().
-- Reuses the same "move profile off personal account" pattern as
-- redeem_invitation, without an invitation token.
-- ============================================================
...
ALTER FUNCTION public.create_platform_account_with_owner(TEXT, UUID, TEXT)
  OWNER TO postgres;
REVOKE ALL ON FUNCTION public.create_platform_account_with_owner(TEXT, UUID, TEXT)
  FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_platform_account_with_owner(TEXT, UUID, TEXT)
  TO authenticated;
